...
(function () {
  ...
  Typo.prototype = {
                  ...
                  for (var k = j + 1; k < _jlen; k++) {
                    ...
                    if (combineRule) {
                      if (
                        ...
                        var otherNewWords = this._applyRule(
                          newWord,
                          combineRule
                        );

                        for (
                          var iii = 0, _iiilen = otherNewWords.length;
                          iii < _iiilen;
                          iii++
                        ) {
      ...
      for (var i = 0, _len = entries.length; i < _len; i++) {
        ...
        if (!entry.match || word.match(entry.match)) {
          ...
          if ("continuationClasses" in entry) {
            for (
              var j = 0, _jlen = entry.continuationClasses.length;
              j < _jlen;
              j++
            ) {
    ...
    check: function (aWord) {
      ...
      var uncapitalizedWord =
        trimmedWord[0].toLowerCase() + trimmedWord.substring(1);
    ...
    hasFlag: function (word, flag, wordFlags) {
      ...
      if (flag in this.flags) {
        if (typeof wordFlags === "undefined") {
          wordFlags = Array.prototype.concat.apply(
            [],
    ...
    suggest: function (word, limit) {
      ...
      for (var i = 0, _len = this.replacementTable.length; i < _len; i++) {
        ...
        if (word.indexOf(replacementEntry[0]) !== -1) {
          var correctedWord = word.replace(
            replacementEntry[0],
            replacementEntry[1]
          );
        ...
        for (var word in words) {
          for (i = 0, _len = word.length + 1; i < _len; i++) {
            ...
            if (s[1]) {
              ...
              var lettercase =
                s[1].substring(0, 1).toUpperCase() === s[1].substring(0, 1)
                  ? "uppercase"
                  : "lowercase";
            ...
            if (s[1]) {
              ...
              for (j = 0; j < alphabetLength; j++) {
                ...
                var lettercase =
                  s[0].substring(-1).toUpperCase() === s[0].substring(-1) &&
                  s[1].substring(0, 1).toUpperCase() === s[1].substring(0, 1)
                    ? "uppercase"
                    : "lowercase";
      ...
      function correct(word) {
        ...
        for (
          i = 0;
          i < Math.min(working_limit, sorted_corrections.length);
          i++
        ) {
          if ("uppercase" === capitalization_scheme) {
            sorted_corrections[i][0] = sorted_corrections[i][0].toUpperCase();
          } else if ("capitalized" === capitalization_scheme) {
            sorted_corrections[i][0] =
              sorted_corrections[i][0].substr(0, 1).toUpperCase() +
              sorted_corrections[i][0].substr(1);
          }

          if (
            !self.hasFlag(sorted_corrections[i][0], "NOSUGGEST") &&
            rv.indexOf(sorted_corrections[i][0]) == -1
          ) {
