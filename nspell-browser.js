// Entry point for bundled nspell
// This will be replaced with the bundled version

// Simple implementation that provides the core API needed by your worker
(function(root) {
  'use strict';
  ...
  function nspell(options) {
    ...
    console.log('Nspell: Initializing with dictionary and affix files');
    this.dictionary = this._parseDictionary(dic);
    this.affixRules = this._parseAffixFile(aff);
    console.log('Nspell: Initialization complete, dictionary size:', Object.keys(this.dictionary).length);
  }
  ...
  // Prototype methods
  nspell.prototype._parseDictionary = function(dic) {
    var dictionary = {};
  ...
  nspell.prototype._parseAffixFile = function(aff) {
    var rules = {
      prefixes: [],
  ...
  nspell.prototype.correct = function(word) {
    ...
    var forms = this._getWordForms(word);
  ...
  nspell.prototype.suggest = function(word) {
    ...
    var edits1 = this._getEditDistance1(word);
    ...
    if (suggestions.length < 5) {
      for (var i = 0; i < Math.min(edits1.length, 50); i++) { // Limit to 50 to avoid excessive processing
        var moreEdits = this._getEditDistance1(edits1[i]);
  ...
  nspell.prototype._getWordForms = function(word) {
    var forms = [];
  ...
  nspell.prototype._getEditDistance1 = function(word) {
    var results = [];
